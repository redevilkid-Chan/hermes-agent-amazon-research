...
一套全自动、多 Agent 协作的亚马逊市场调研流水线。系统自动接入品类原始数据（卖家精灵数据），执行竞品分析、生成 BCG 矩阵，并产出数据驱动的市场战略建议——全程由 Hermes Agent 编排调度。
...
└──────────┘    └──────────┘    └──────────┘    └──────────┘
 卖家精灵    明星/金牛       五维度决策       Dark/Noir/SaaS
 卖家精灵       问题/瘦狗       Devil's Advocate 42页专业投影片
